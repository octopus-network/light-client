    ...
    H: hash_db::Hasher<Out = H256> + Debug + 'static,
{
    ...
    let binding = Vec::<u8>::from(proof.clone());
    let mut raw_proof = binding.as_slice();
    let trie_proof: Vec<Vec<u8>> = codec::Decode::decode(&mut raw_proof)
    ...
    H: hash_db::Hasher<Out = H256> + Debug + 'static,
{
    ...
    let trie_proof: Vec<Vec<u8>> =
        codec::Decode::decode(&mut raw_proof).map_err(anyhow::Error::msg)?;
    let proof = StorageProof::new(trie_proof);
    let root = H256::from_slice(root.as_bytes());
    let child_info = ChildInfo::new_default(prefix.as_bytes());
...
where
    H: Clone,
    C: ClientReader + ChannelReader,
{
    let current_time = ClientReader::host_timestamp(ctx)?;
    let current_height = ClientReader::host_height(ctx)?;
